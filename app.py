...
app = Chalice(app_name='reacjilator')
...
def translate_lambda_function(event, context):
    ...
    lang_yandex = lang.get_yandex(lang_name)
    if lang_yandex is None:
        app.log.error('Not found language yandex value.')
        return
    ...
    # message translate by yandex
    tr_lang, tr_msg = translate_by_yandex(slack_msg_text, lang_yandex)
...
def receive_slack_events():
    ...
    if slack_token is None or slack_token != SLACK_VERIFICATION_TOKEN:
...
def translate_by_yandex(msg, lang):
    url = "https://translate.yandex.net/api/v1.5/tr.json/translate"
    headers = {"Accept": "application/json"}
    params = {"key": YANDEX_API_KEY, "text": msg, "lang": lang}

    response = requests.get(url, params=params, headers=headers)

    # debug
    app.log.debug('yandex response:{}'.format(response))

    if response.json().get('code') != 200:
        raise create_response(400, 'Could not translate it.')

    tr_lang = response.json().get('lang')
